...
if (isset($_POST['message_minichat']) and !empty($_POST['message_minichat'])) {
    ...
    $req_verif_next_mail->execute();

    if (!$res_verif_next_mail = $req_verif_next_mail->fetch()) {
...
// Insertion du mail dans la BDD
            $req_create_mail = $bdd->prepare('INSERT INTO mails (forID, mail_subject, mail_message, date_mail, date_next_mail) VALUES (184, "' . $mail_subject . '", "' . $mail_message . '", CURRENT_TIMESTAMP(), (CURRENT_TIMESTAMP() + (7 * 3600)))');
            $req_create_mail->execute();

            echo '<br>Message et mail envoyés avec succès à ONLYWEB. Redirection en cours...';
    ...
    // Si l'utilisateur a déjà envoyé un mail de part l'envoi de son message
    elseif ($res_verif_next_mail = $req_verif_next_mail->fetch()) {
        // On sécurise les données
        $res_verif_next_mail['date_mail'] = htmlspecialchars($res_verif_next_mail['date_mail']);
        $res_verif_next_mail['date_next_mail'] = htmlspecialchars($res_verif_next_mail['date_next_mail']);

        // On en fait des variables plus lisibles
        $date_mail = $req_verif_next_mail['date_mail'];
        $date_next_mail = $req_verif_next_mail['date_next_mail'];

        // Si la date du premier mail est inférieure à celle du prochain
        if ($date_mail < $date_next_mail) {
            // Préparation puis execution de la requête pour sortir les infos pour l'envoi du mail
            $req_mail_infos = $bdd->prepare('SELECT mail FROM membres WHERE id = "' . $_POST['authorID_msg_minichat'] . '"');
            $req_mail_infos->execute();
